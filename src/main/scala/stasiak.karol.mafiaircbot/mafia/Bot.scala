...
class Bot(val sendPrivateMessage: (String, Seq[String])=>Unit) {
  ...
  def advanceTime(): List[String] = game match {
    ...
    case Some(g) =>
      if (g.canSwitchPhase() == false) {
        ...
        phaseResults ++ gameResults ++ advanceTime()
  ...
  def end(poker: String) = game match{
    ...
    case Some(g) => g.status match {
      ...
      case Running => 
        if(g.signedupPlayers.contains(poker)){
          if(g.endGameAt<System.currentTimeMillis()){
            ("Phase sped up because some players were tardy and didn't submit their night actions in time.") +: advanceTime()
          }else {
            g.askedForEnding += poker
            endInternal(g)
            val players = if(g.askedForEnding.size==1) "1 player" else g.askedForEnding.size+" players"
            val timeLeft = (g.endGameAt-System.currentTimeMillis())/1000
            List(s"$players asked to speed up the currect phase. Please reply with !end after ${timeLeft} seconds pass to force the next phase.")
          }
        }else{
